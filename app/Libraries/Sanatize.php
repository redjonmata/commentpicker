...
/**
 * Sanatize input to be used in search result 
 * 
...
'\x{ff5b}-\x{ff65}\x{ff70}\x{ff9e}\x{ff9f}\x{ffe0}-\x{fffd}';
	...
	/* Constants */
	private static $ATOM = "Y-m-d\TH:i:sP";
	private static $COOKIE = "l, d-M-Y H:i:s T";
	private static $ISO8601 = "Y-m-d\TH:i:sO";
	private static $RFC822 = "D, d M y H:i:s O";
	private static $RFC850 = "l, d-M-y H:i:s T";
	private static $RFC1036 = "D, d M y H:i:s O";
	private static $RFC1123 = "D, d M Y H:i:s O";
	private static $RFC2822 = "D, d M Y H:i:s O";
	private static $RFC3339 = "Y-m-d\TH:i:sP";
	private static $RFC3339_EXTENDED = "Y-m-d\TH:i:s.vP";
	private static $RSS = "D, d M Y H:i:s O";
	private static $W3C = "Y-m-d\TH:i:sP";

	/**
	 * 	Blacklisted words to remove from search
	 * @var string
	 */
	public static $EM_SEARCH_BLACKLIST = '4r5e';

	/**
	 * 	minimum characters to start performing the search
	 * @var integer
	 */
	public static $EM_SEARCH_MIN_WORD_LENGTH = 1;

	/**
	 * maximium characters used per word search
	 * @var integer
	 */
	public static $EM_SEARCH_MAX_WORD_LENGTH = 30;
	...
	public static function sanitize($string = '') {
	    ...
	    $string = preg_replace('/(['.self::PREG_CLASS_NUMBERS.']+)['.self::PREG_CLASS_PUNCTUATION.']+(?=['.self::PREG_CLASS_NUMBERS.'])/u', '\1', $string);
	    $string = preg_replace('/['.self::PREG_CLASS_SEARCH_EXCLUDE.']+/u', ' ', $string);
	    ...
	    if (!empty($blacklist)) {
	    	...
	    	if (is_array($blacklist)) {
	    		...
	    		$blacklist = array_map(array(__CLASS__, 'lowercase'), $blacklist);
	    	} else {
	    		$blacklist = [$blacklist];
	    	}
	    ...
	    $string = self::replaceAccentedChars(trim(preg_replace('/\s+/', ' ', $string)));
	    return $string;
	}
